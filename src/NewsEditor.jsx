  ...
  const addNews = async () => {
    try {
      if (selectedId) {
        ...
        fileInput.current.value = ''; // 파일 입력 초기화
      }
    } catch (error) {
      console.error(error);
      setError('뉴스 기사를 추가하는 도중 에러가 발생했습니다. 빈 항목을 작성해 주세요');
    }
  };
  ...
  return (
    <Box maxW="600px" mx="auto" my="4">
      ...
      <FormControl my="4">
        ...
        <Input type="file" ref={fileInput} onChange={handleMainImageChange} accept="image/*" />
      </FormControl>
      ...
      <Box textAlign="center" fontSize="18px" fontWeight="bold">
        {news.map((news) => (
          <Box key={news.id} borderBottom="1px solid #ccc" py="4">
            <Box fontWeight="normal">{news.createdAt}</Box>
